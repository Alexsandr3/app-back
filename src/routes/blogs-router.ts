...
blogsRouter.get('/:blogId/posts', checkBlogIdValidForMongodb, pageValidations, getUserIdFromAccessTokena, async (req: RequestWithParamsAndQeury<{ blogId: string }, QeuryParams_GetPostsModel>, //+++
    ...
    const posts = await blogsQueryRepositories.findPostsByIdBlog(blogId, dataForReposit, req.userId)
    if (!posts) {
        res.sendStatus(HTTP_STATUSES.NOT_FOUND_404)
        return;
    }
